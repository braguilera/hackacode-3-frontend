...
import { Trash2, Edit3, Stethoscope, DollarSign } from 'lucide-react';
...
const CardMedico = ({ dataMedico, onEdit, onDelete }) => {
  ...
  useEffect(() => {
    if (dataMedico && dataMedico.id) {
      getDatos('/api/consultas', 'Error fetching consultations')
        .then((consultas) => {
          const count = consultas.filter(
            (consulta) => Number(consulta.medicoId) === Number(dataMedico.id)
          ).length;
        ...
        <footer className="grid grid-cols-2 divide-x border-t h-20">
          <section className="p-4 flex items-center gap-2 h-full">
            ...
            <article>
              ...
              <p className="font-semibold text-gray-800 w-20 truncate">{consultasCount}</p>
            </article>
          </section>
