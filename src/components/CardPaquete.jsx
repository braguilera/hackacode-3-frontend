...
import { Trash2, Edit3, User, DollarSign, Package} from 'lucide-react';
...
className='relative bg-white rounded-xl shadow-sm hover:shadow-md transition-all duration-300 p-4 border border-gray-100 group cursor-pointer select-none '
        ...
        <article className="space-y-2 mb-3">
            ...
            <div key={index} className="flex justify-between items-center text-sm p-2 bg-blue-50 rounded-lg">
                <p className="text-gray-600">{servicio.nombre}</p>
                <p className="text-green-500 font-medium">${servicio.precio}</p>
            </div>
            ))}
        </article>
        ...
        <footer className="pt-2 border-t border-gray-100">
            <div className="flex justify-between items-center mt-2">
                ...
                <p className="text-lg font-bold text-green-600">${dataPaquete.precio}</p>
            </div>
        </footer>
    </motion.main>
);  
